...
class GeneradorDTEBase:
    ...
    def _crear_elemento_encabezado(self):
        ...
        # IndServicio: OBLIGATORIO para boletas (tipo 39/41) segun SII
        if self.tipo_dte in [39, 41]:
            etree.SubElement(id_doc, 'IndServicio').text = '3'
        
        # FmaPago: PROHIBIDO para boletas (tipo 39/41) - solo para facturas y otros
        if self.forma_pago and self.tipo_dte not in [39, 41]:
            etree.SubElement(id_doc, 'FmaPago').text = str(self.forma_pago)
        
        if self.fecha_vencimiento and self.tipo_dte not in [39, 41]:
        ...
        if self.tipo_dte in [39, 41]:
            etree.SubElement(emisor, 'RznSocEmisor').text = validar_texto(self.razon_social_emisor, 'Razón Social Emisor', max_length=100)
            etree.SubElement(emisor, 'GiroEmisor').text = validar_texto(self.giro_emisor, 'Giro Emisor', max_length=80, requerido=False)
        else:
        ...
        # Acteco - NO incluir en boletas
        if self.tipo_dte not in [39, 41] and self.acteco:
        ...
        if self.rut_receptor and validar_rut(self.rut_receptor):
            ...
            # GiroRecep - NO incluir en boletas (tipo 39, 41)
            if self.tipo_dte not in [39, 41] and self.giro_receptor:
        ...
        # TasaIVA e IVA - NO incluir en boletas (tipo 39)
        if self.tipo_dte not in [39, 41] and self.iva > 0:
