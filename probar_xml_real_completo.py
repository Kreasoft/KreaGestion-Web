...
# -*- coding: utf-8 -*-
"""
Script para probar DTEBox con el XML real proporcionado
"""
import os
import sys
import django
...
def probar_xml_real():
    """
    Prueba el XML real con DTEBox
    """
    print("=" * 80)
    print("PRUEBA DTEBox CON XML REAL DE DTE")
    print("=" * 80)
    print()
    
    print("Información del XML:")
    print(f"  Tipo: EnvioDTE completo")
    ...
    try:
        ...
        root = etree.fromstring(XML_REAL.encode('ISO-8859-1'))
        
        ns_sii = "http://www.sii.cl/SiiDte"
        
        # Buscar datos del DTE
        rut_emisor = root.find(f'.//{{{ns_sii}}}RutEmisor')
        tipo_dte = root.find(f'.//{{{ns_sii}}}TipoDTE')
        folio = root.find(f'.//{{{ns_sii}}}Folio')
        fch_resol = root.find(f'.//{{{ns_sii}}}FchResol')
        nro_resol = root.find(f'.//{{{ns_sii}}}NroResol')
        
        print("Datos extraídos del XML:")
        if rut_emisor is not None:
            print(f"  RUT Emisor: {rut_emisor.text}")
        if tipo_dte is not None:
            print(f"  Tipo DTE: {tipo_dte.text}")
        if folio is not None:
            print(f"  Folio: {folio.text}")
        if fch_resol is not None:
            print(f"  Fecha Resolución: {fch_resol.text}")
        if nro_resol is not None:
            print(f"  Número Resolución: {nro_resol.text}")
        print()
    except Exception as e:
        print(f"[ADVERTENCIA] No se pudo parsear XML para extraer info: {str(e)}")
        print()
    ...
    try:
        ...
        # Configurar resolución si no existe
        if not empresa.resolucion_numero or not empresa.resolucion_fecha:
        ...
        # 1. Extraerá el DTE del EnvioDTE
        # 2. Removerá el TED timbrado y lo dejará vacío
        # 3. Enviará a DTEBox
        
        print("Enviando XML real a DTEBox...")
        print("(El servicio extraerá el DTE y removerá el TED timbrado)")
        print("=" * 80)
        
        resultado = dtebox_service.timbrar_dte(XML_REAL)
        ...
        if resultado['success']:
            ...
            print("-" * 80)
            print()
            print("El TED puede ser insertado en el XML del DTE para completar el timbraje.")
        else:
            print("[ERROR] Error al timbrar DTE")
            print(f"Error: {resultado['error']}")
            print()
            print("Archivos de debug guardados en:")
