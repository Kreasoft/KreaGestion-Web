...
class Command(BaseCommand):
    help = 'Corregir valores del ticket #000090'

    def handle(self, *args, **options):
        try:
            # Buscar el ticket
            ticket = Venta.objects.get(numero_venta='000090', tipo_documento='vale')
            
            self.stdout.write(f"\n=== TICKET #000090 - VALORES ACTUALES ===")
            ...
            self.stdout.write(f"\n=== TICKET #000090 - VALORES CORREGIDOS ===")
            self.stdout.write(f"Subtotal: {ticket.subtotal}")
            self.stdout.write(f"Neto: {ticket.neto}")
            self.stdout.write(f"IVA: {ticket.iva}")
            self.stdout.write(f"Impuesto Específico: {ticket.impuesto_especifico}")
            ...
            self.stdout.write(self.style.SUCCESS(f'\n✓ Ticket #000090 corregido exitosamente'))
            
        except Venta.DoesNotExist:
            self.stdout.write(self.style.ERROR('✗ Ticket #000090 no encontrado'))
